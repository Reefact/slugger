...
public sealed class OptionPrecedenceTests : IDisposable {
    ...
    private const string NounOld       = "zog[a-z]{2}";
    ...
    public void A_run_that_asks_for_nothing_is_shaped_by_everything_init_saved() {
        ...
        // Verify
        Assert.Matches($"^{Adjective}={Participle}={NounOld}[0-9a-f]{{3}}$", Assert.Single(slugs));
    }
    ...
    public void An_option_on_the_command_line_overrules_the_same_option_saved() {
        ...
        // Verify
        Assert.Matches($"^{Adjective}#{NounOld}#[0-9]$", Assert.Single(slugs));
    }
    ...
    public void An_option_the_command_line_leaves_out_still_comes_from_what_was_saved() {
        ...
        // Verify
        Assert.Matches($"^{Adjective}@{NounOld}[0-9a-f]{{2}}$", Assert.Single(slugs));
    }
    ...
    public void A_casing_on_the_command_line_overrules_the_saved_one() {
        ...
        // Verify
        Assert.Matches($"^{Adjective}-{Participle}-{NounOld}$", Assert.Single(slugs));
    }
    ...
    public void A_token_chance_on_the_command_line_brings_the_token_back() {
        ...
        // Verify
        Assert.Matches($"^{Adjective}-{Participle}-{NounOld}-[0-9]{{3}}$", Assert.Single(slugs));
    }
    ...
    public void The_drawn_themes_own_style_beats_what_init_saved() {
        ...
        // Verify
        Assert.Matches($"^{Adjective}_{NounOld}$", Assert.Single(slugs));
    }
    ...
    public void A_second_init_keeps_what_the_first_one_saved() {
        ...
        // Verify
        Assert.Equal(2, slugs.Count);
        Assert.All(slugs, slug => Assert.Matches($"^{Adjective}={NounOld}[0-9a-f]{{2}}$", slug));
    }
    ...
    public void A_second_init_replaces_the_options_it_names_again() {
        ...
        Assert.Matches($"^{Adjective}#{Participle}#{NounOld}#[0-9]{{2}}$", Assert.Single(slugs));
    }
