...
/// <summary>
///     The chain of DEC0004: explicit argument, then the theme's defaults, then the config
...
public sealed class OptionResolverTests {
    ...
    public void Falls_back_to_the_programs_own_default_when_nobody_says_anything() {
        // Exercise
        GenerationOptions options = OptionResolver.Resolve(SluggerOptions.Empty, null, ThemeWithDefaults(ThemeDefaults.Empty), 1);
    ...
    public void The_saved_config_beats_the_programs_default() {
        ...
        // Exercise
        GenerationOptions options = OptionResolver.Resolve(SluggerOptions.Empty, saved, ThemeWithDefaults(ThemeDefaults.Empty), 1);
    ...
    public void The_themes_defaults_beat_the_saved_config() {
        ...
        // Exercise
        GenerationOptions options = OptionResolver.Resolve(SluggerOptions.Empty, saved, Styled, 1);
    ...
    public void An_explicit_argument_beats_everything() {
        ...
        // Exercise
        GenerationOptions options = OptionResolver.Resolve(commandLine, saved, Styled, 1);
    ...
    public void An_empty_word_separator_is_an_answer_and_outranks_the_layer_below() {
        ...
        // Exercise
        GenerationOptions fromTheme = OptionResolver.Resolve(SluggerOptions.Empty, saved, glues, 1);
        GenerationOptions overruled = OptionResolver.Resolve(new SluggerOptions { WordSeparator = "_" }, saved, glues, 1);
    ...
    public void Nobody_saying_anything_leaves_the_separator_to_join_the_words_too() {
        // Exercise
        GenerationOptions options = OptionResolver.Resolve(SluggerOptions.Empty, null, Styled, 1);
    ...
    ///     --theme heroku already reproduces heroku's style (DEC0004).
    /// </summary>
    [Fact]
    public void Absent_the_flag_a_single_theme_applies_its_own_style() {
        Assert.True(OptionResolver.AppliesTheStyleOf(null, 1));
    }

    [Fact]
    public void Absent_the_flag_several_themes_keep_one_consistent_format() {
        Assert.False(OptionResolver.AppliesTheStyleOf(null, 3));
    }

    [Fact]
    public void Auto_is_the_same_as_not_passing_the_flag() {
        Assert.True(OptionResolver.AppliesTheStyleOf(MimicStyle.Auto, 1));
        Assert.False(OptionResolver.AppliesTheStyleOf(MimicStyle.Auto, 3));
    }

    [Fact]
    public void Force_applies_the_drawn_themes_style_even_among_several() {
        Assert.True(OptionResolver.AppliesTheStyleOf(MimicStyle.Force, 3));
    }

    [Fact]
    public void Off_refuses_the_style_even_for_a_single_theme() {
        Assert.False(OptionResolver.AppliesTheStyleOf(MimicStyle.Off, 1));
    }

    [Fact]
    public void An_explicit_argument_still_wins_when_mimic_style_is_forced() {
        // Setup - the worked example of DEC0004: --mimic-style false --sep = ignores the theme, then forces =.
        SluggerOptions commandLine = new() { Separator = '=', MimicStyle = MimicStyle.Off };

        // Exercise
        GenerationOptions options = OptionResolver.Resolve(commandLine, null, Styled, 1);
    ...
    public void Merging_with_no_saved_config_changes_nothing() {
        ...
        // Exercise
        SluggerOptions merged = OptionResolver.Merge(commandLine, null);
